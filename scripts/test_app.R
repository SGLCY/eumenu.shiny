...
ggplot2::theme_set(theme_light(16))
...
server <- function(input, output) {
  ...
  filter_fdgrp <- function(.data, food_group){
    ...
    .data %>% 
      filter(fdx1_name %in% food_group | fdx2_name  %in% food_group | ENFOODNAME %in% food_group) %>%
  ...
  filters <- reactive(c(input$fdx1, input$fdx2, input$ENFOODNAME))
  ...
  cons_mean <- reactive({
    ...
    output$fdx1_slct <- renderUI({
      ...
      selectInput("fdx1","Select FoodEx1 name(s)", choices = fdx1_names, multiple = TRUE)
    })
    
    output$fdx2_slct <- renderUI({
      fdx2_names <- unique(dataset()$fdx2_name)
      selectInput("fdx2","Select FoodEx2 name(s)", choices = fdx2_names, multiple = TRUE)
    })
    
    output$ENFOODNAME <- renderUI({
      ENFOOD_names <- unique(dataset()$ENFOODNAME)
      selectInput("ENFOODNAME","Select English Food name(s)", choices = ENFOOD_names, multiple = TRUE)
    })
